"""ExcelLocalDataSet loads and saves data to a local Excel file. 
The underlying functionanlity is supported by pandas, so it supports 
all allowed pands options for loading and saving Excel files.
"""
...
class ExcelLocalDataSet(AbstractDataSet):
    """``ExcelLocalDataSet`` loads and save data to local Excel file. The
